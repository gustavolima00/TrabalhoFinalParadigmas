...
toValue n precision
    | null (tail (splitOn "." n)) = (10^precision)*(read (head (splitOn "." n)) :: Integer)
    | otherwise = 
...
printSeq (arg:args) f s w
    -- One arg
    | null args && (read arg :: Double) < 1 = 
        printf ""
    | null args = 
        printSeqHandler f s w "1" "1" arg
        
    -- Two args
    | null (tail args) && (read arg :: Double) > (read (head args) :: Double) = 
        printf ""
    | null (tail args) = 
        printSeqHandler f s w arg "1" (head args)
    
    -- Tree args
    |  null (tail (tail args)) && (read arg :: Double) > (read (head (tail args)) :: Double) = 
        printf ""
    | null (tail (tail args)) = 
